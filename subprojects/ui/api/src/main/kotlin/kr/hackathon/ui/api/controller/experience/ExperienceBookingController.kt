...
class ExperienceBookingController(
    ...
    @Operation(summary = "예약 내역 조회", description = "로그인한 사용자의 체험 예약 내역을 조회합니다. 로그인 세션이 필요합니다.")
    @GetMapping
    fun getMyBookings(
        session: HttpSession,
        ...
        return ResponseEntity.ok(ApiResponse.success(
            experienceBookingService.getMyBookings(sessionUser.id)
        ))
    }

    @Operation(summary = "완료 내역 조회", description = "로그인한 사용자의 완료된 체험 예약 내역을 조회합니다. 로그인 세션이 필요합니다.")
    @GetMapping("/completed")
    fun getMyCompletedBookings(
        session: HttpSession,
    ): ResponseEntity<ApiResponse<*>> {
        val sessionUser = session.getAttribute(SESSION_USER_KEY) as? SessionUser
            ?: return ResponseEntity.status(401).body(ApiResponse.error(
        ...
        return ResponseEntity.ok(ApiResponse.success(
            experienceBookingService.getMyCompletedBookings(sessionUser.id)
        ))
    }
